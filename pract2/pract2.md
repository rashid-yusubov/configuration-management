...
1. **Скачать архив с исходным кодом**:
   ...
   -  Перейти на страницу релизов Matplotlib на GitHub и скачайть последнюю версию архива с исходным кодом.

2. **Распакуйте архив**:

   -  Использовать команду `tar -xzf имя_файла.tar.gz` для распаковки архива (или `unzip имя_файла.zip` для файлов `.zip`).
...
Используйте команду cd имя_директории, чтобы перейти в директорию, содержащую файл setup.py.
Установите пакет:
Выполните команду python setup.py install, чтобы установить пакет.
```
## Результат:
